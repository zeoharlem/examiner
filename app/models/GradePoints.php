...
class GradePoints extends Phalcon\Mvc\Model{
    ...
    const FIRSTCLASS = 4, UPPERCLASS = 3.5, SECONDCLASS = 2.4, THIRDCLASS = 1.9, FAILCLASS = 1;
    ...
    //Get Class Divisions
    public static function __classDivision($weightedAvr){
        if($weightedAvr >= self::FIRSTCLASS){
            return 'FIRST CLASS';
        }
        elseif($weightedAvr >= self::UPPERCLASS && $weightedAvr < self::FIRSTCLASS){
            return 'SECOND CLASS UPPER DIVISION';
        }
        elseif($weightedAvr >= self::SECONDCLASS && $weightedAvr < self::UPPERCLASS){
            return 'SECOND CLASS LOWER DIVISION';
        }
        elseif($weightedAvr >= self::THIRDCLASS && $weightedAvr < self::SECONDCLASS){
            return 'THIRD CLASS DIVISION';
        }
        elseif($weightedAvr >= self::FAILCLASS && $weightedAvr < self::THIRDCLASS){
            return 'PASSED';
        }
    }
